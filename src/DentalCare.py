...
def main_menu():
    ...
    while True:
        ...
        if op == "1":
            ...
            clinic.patients.append(p)
            clinic.patient_hash[p.patient_id] = p
            ...
            if delete_patient_db(pid):
                clinic.patients = [x for x in clinic.patients if x.patient_id != pid]
            ...
            clinic.create_appointment(pid, date, service, price)
            ...
            clinic.mark_attended(aid)

        elif op == "9":
            aid = int(input("ID cita a cancelar: "))
            clinic.cancel_appointment(aid)
